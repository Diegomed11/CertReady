// Command lambda adapta el servicio content para AWS Lambda (ruta de costo cero).
package main
...
import (
	...
	"github.com/certready/certready/services/content/internal/config"
	"github.com/certready/certready/services/content/internal/httpapi"
...
func main() {
	cfg, err := config.Load()
	if err != nil {
		logging.New("content", "dev").Error("configuración inválida", "err", err.Error())
		panic(err)
	}
	...
	logger.Info("lambda content inicializada", "version", cfg.Version, "env", cfg.Env)

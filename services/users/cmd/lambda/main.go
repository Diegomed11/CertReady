// Command lambda adapta el servicio users para AWS Lambda (ruta de costo cero).
package main
...
import (
	...
	"github.com/certready/certready/services/users/internal/httpapi"
...
func main() {
	cfg, err := config.Load()
	if err != nil {
		logging.New("users", "dev").Error("configuración inválida", "err", err.Error())
		panic(err)
	}
	...
	logger.Info("lambda users inicializada", "version", cfg.Version, "env", cfg.Env)
